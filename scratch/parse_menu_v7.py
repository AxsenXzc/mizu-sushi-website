...
# Global categories (exclude gamberi, manzo, pollo which can be names/parts of names)
categories = {
    "uramaki fritto - 8pz", "uramaki speciale - 8pz", "uramaki venere - 8pz", "uramaki pink - 8pz",
    "tempura - fritto", "piastra",
    "dolci non compresi nell’offerta di 22,00 €", "spaghetti giapponesi", "spaghetti cinesi",
    "ramen", "riso", "uramaki- 8pz", "antipasti", "insalata", "zuppe", "tartare",
...
restricted_categories = {"gamberi", "manzo", "pollo", "pollo e verdure"}
...
for idx, line in enumerate(lines):
    ...
    if line.lower() in categories:
        is_cat = True
    elif line.lower() in restricted_categories and (380 <= idx <= 400):
        is_cat = True
...
for idx, el in enumerate(elements):
    ...
    if el["type"] == "NAME":
        start_new = False
        if current_item:
            # Bug fix: Only start new item if current_item has name_parts AND (desc_parts or price or number)
            if current_item["name_parts"] and (current_item["desc_parts"] or current_item["price"] is not None or current_item["number"] is not None):
                start_new = True
...
with open(r"c:\Users\anelk\Desktop\Sito Web Mizu Sushi\scratch\parsed_menu_v7.json", 'w', encoding='utf-8') as f:
