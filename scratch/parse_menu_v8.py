...
for idx, el in enumerate(elements):
    ...
    if el["type"] == "NAME":
        start_new = False
        if current_item:
            # We ONLY start a new item on NAME if current_item has name_parts AND (desc_parts or price)
            # We do NOT start a new item on name if the current item only has a number and name, but no desc or price!
            if current_item["name_parts"] and (current_item["desc_parts"] or current_item["price"] is not None):
                start_new = True
...
with open(r"c:\Users\anelk\Desktop\Sito Web Mizu Sushi\scratch\parsed_menu_v8.json", 'w', encoding='utf-8') as f:
